...
/* ── BOUNCING MOLECULE (shows after completion!) ── */
function BouncingMolecule({ required, arenaRef }) {
  const ref = useRef(null);
  const posRef = useRef({ x: 100, y: 80 });
  const velRef = useRef({ x: rand(0.8, 1.6) * (Math.random()>0.5?1:-1), y: rand(0.8, 1.6) * (Math.random()>0.5?1:-1) });
  const animRef = useRef(null);

  const atomsList = [];
  Object.entries(required).forEach(([el, count]) => { for (let i = 0; i < count; i++) atomsList.push(el); });
  const moleculeWidth = atomsList.length * 38 + 20;
  const moleculeHeight = 60;
  ...
  useEffect(() => {
    const go = () => {
      ...
      const b = arena.getBoundingClientRect();
      posRef.current.x += velRef.current.x; posRef.current.y += velRef.current.y;
      if (posRef.current.x <= 0 || posRef.current.x >= b.width - moleculeWidth) { velRef.current.x *= -1; posRef.current.x = Math.max(0, Math.min(b.width - moleculeWidth, posRef.current.x)); }
      if (posRef.current.y <= 0 || posRef.current.y >= b.height - moleculeHeight) { velRef.current.y *= -1; posRef.current.y = Math.max(0, Math.min(b.height - moleculeHeight, posRef.current.y)); }
      ref.current.style.transform = `translate(${posRef.current.x}px, ${posRef.current.y}px)`;
      animRef.current = requestAnimationFrame(go);
    };
    animRef.current = requestAnimationFrame(go);
    return () => { if (animRef.current) cancelAnimationFrame(animRef.current); };
  }, [arenaRef, moleculeWidth, moleculeHeight]);

  return (
    <div ref={ref} style={{
      position: "absolute", display: "flex", alignItems: "center", gap: 0,
          ...
          <div key={i} style={{ position: "relative", display: "flex", alignItems: "center" }}>
            {i > 0 && (
              <div style={{
                ...
                marginRight: -2, marginLeft: -2, zIndex: 1,
            ...
            <div style={{
              ...
              boxShadow: `0 0 16px ${d.color}66, inset 0 -2px 4px ${d.glow}44`,
            }}>
              <div style={{ position:"absolute",top:4,left:7,width:12,height:6,borderRadius:"50%",background:"rgba(255,255,255,0.35)",transform:"rotate(-25deg)" }} />
              <span style={{ fontSize: 16, fontWeight: 800, color: "#1a1a2e", fontFamily: "var(--font)", lineHeight: 1 }}>{el}</span>
              <span style={{ fontSize: 6, color: "#1a1a2eaa", fontWeight: 700 }}>{d.number}</span>
            </div>
          </div>
        );
      })}
      ...
      {screen==="compound-play" && level && (
        <div style={{ display:"flex",flexDirection:"column",height:"100vh",padding:"8px 10px 10px",position:"relative",zIndex:1 }}>
          ...
          <div ref={arenaRef} style={{ flex:1,position:"relative",borderRadius:20,background:"rgba(255,255,255,0.02)",border:"2px solid rgba(255,255,255,0.04)",overflow:"hidden",minHeight:160 }}>
            ...
            {showMolecule && <BouncingMolecule required={level.required} arenaRef={arenaRef} />}
          </div>
      ...
      {screen==="compound-result" && level && levelResult && (
        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",minHeight:"100vh",padding:"20px 16px",animation:"slide-in 0.5s ease-out",textAlign:"center",position:"relative",zIndex:1,overflowY:"auto" }}>
          {levelResult.won ? (<>
            ...
            <div style={{ fontSize:22,fontWeight:800,marginBottom:14,background:"linear-gradient(90deg,#FF6B6B,#4ECDC4)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent" }}>{level.formula}</div>
